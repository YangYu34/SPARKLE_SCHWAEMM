...
def SCHWAEMM_192_192(eng, M_value, Key, Nonce, Auth_value, len):
    # carry bit
    ...
    N = eng.allocate_qureg(192)
  
    if(resource_check != 1):
        RC_XOR(eng, Key, K, 192)
    if (resource_check != 1):
        RC_XOR(eng, Nonce, N, 192)

    # N || K
    concat_NK(eng, K, S[:192], 192)
    concat_NK(eng, N, S[192:384], 192)
    ...
    ## AD 계산
    X | S[27] # y[5] ^= CONSTA, 1000(0x8)

    p1(eng, S, A_last, carry)

    ## SL ^ SR
    for i in range(32):
        CNOT | (S[191 - i], S[383 - i])
        CNOT | (S[159 - i], S[351 - i])
        CNOT | (S[127 - i], S[319 - i])
        CNOT | (S[95 - i], S[287 - i])
        CNOT | (S[63 - i], S[255 - i])
        CNOT | (S[31 - i], S[223 - i])
...
def FeistelSwap(eng, s):
    for i in range(32):
        Swap | (s[287-i], s[383-i]) #y1 <-> x0
        Swap | (s[255-i], s[351-i]) #x2 <-> y0
        Swap | (s[223-i], s[319-i]) #y2 <-> x1
    #
    for i in range(32):
        CNOT | (s[383 - i], s[287 - i])  # y1 ^= x0
        CNOT | (s[351 - i], s[255 - i])  # x2 ^= y0
        CNOT | (s[319 - i], s[223 - i])  # y2 ^= x1
...
def p1(eng, s, d, carry):
    ...
    for i in range(32):
        CNOT | (d[31 - i], s[383 - i])  # x0 ^= AM[0]
    X | (s[327])

def SPARKLE_384(eng, x, carry, n):
...
def L6(eng, x0, x1, x2, x3, x4, x5, y0, y1, y2, y3, y4, y5):
    ...
    XOR(eng, y0, y3, 32) # y3 = y3 ^ y0

    XOR(eng, x0[16:32], y4[0:16], 16)  # y4 = y4 ^ tx
    XOR(eng, x0[0:16], y4[16:32], 16)  # y4 = y4 ^ tx
    XOR(eng, y1, y4, 32) # y4 = y4 ^ y1

    XOR(eng, x0[16:32], y5[0:16], 16)  # y5 = y5 ^ tx
    XOR(eng, x0[0:16], y5[16:32], 16)  # y5 = y5 ^ tx
    XOR(eng, y2, y5, 32)  # y5 = y5 ^ y2
    ...
    XOR(eng, x0, x3, 32) # x3 = x3 ^ x0

    XOR(eng, y0[16:32], x4[0:16], 16)  # x4 = x4 ^ ty
    XOR(eng, y0[0:16], x4[16:32], 16)  # x4 = x4 ^ ty
    XOR(eng, x1, x4, 32) # x4 = x4 ^ x1

    XOR(eng, y0[16:32], x5[0:16], 16)  # x5 = x5 ^ ty
    XOR(eng, y0[0:16], x5[16:32], 16)  # x5 = x5 ^ ty
    XOR(eng, x2, x5, 32)  # x5 = x5 ^ x2
...
SCHWAEMM_192_192(eng, 0x03020100, 0x17161514131211100F0E0D0C0B0A09080706050403020100, 0x17161514131211100F0E0D0C0B0A09080706050403020100, 0x03020100, 32)
eng.flush()
